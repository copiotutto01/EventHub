...
# =========================================================================
# 🌟 ENDPOINT AGGIORNATO: CHECKOUT STRIPE (Risolto bug parametro 'title')
# =========================================================================
@events_bp.route('/api/events/<int:event_id>/checkout', methods=['POST'])
@token_required
def checkout_event(event_id):
    event = Event.query.get_or_404(event_id)
    user_id = request.user.get('sub') or request.user.get('id')

    # Controllo disponibilità biglietti
    available = event.max_tickets - event.tickets_sold
    if available <= 0:
        return jsonify({'message': 'Biglietti esauriti per questo evento!'}), 400

    # Controllo se l'utente è già iscritto all'evento
    existing_reg = db.session.query(event_registrations).filter_by(event_id=event_id, user_id=str(user_id)).first()
    if existing_reg:
        return jsonify({'message': 'Sei già iscritto a questo evento!'}), 400

    # SE L'EVENTO È GRATUITO (0€): Iscrizione diretta immediata senza passare da Stripe
    if float(event.price) <= 0.0:
        try:
            insert_stmt = event_registrations.insert().values(
                event_id=event.id,
                user_id=str(user_id),
            ...
            db.session.commit()
            return jsonify({'requires_stripe': False, 'message': 'Iscrizione gratuita registrata con successo!'}), 200
        except Exception as e:
            db.session.rollback()
            return jsonify({'message': f'Errore iscrizione gratuita: {str(e)}'}), 500

    # SE L'EVENTO È A PAGAMENTO: Generiamo la sessione sicura di Stripe Checkout
    try:
        # Recupera l'URL di base dell'applicazione frontend per i reindirizzamenti di Stripe
        frontend_url = request.headers.get('Origin') or "https://symmetrical-fishstick-4jxj6vp7qq5wc7wp-5173.app.github.dev"
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            line_items=[{
                'price_data': {
                    'currency': 'eur',
                    'product_data': {
                        'name': event.title,  # <-- Sostituito con successo 'title' con 'name'
                        'description': event.description[:100] if event.description else 'Biglietto Evento',
                    },
                    'unit_amount': int(float(event.price) * 100), # Stripe richiede i centesimi
                },
                'quantity': 1,
            }],
            mode='payment',
            success_url=f"{frontend_url}?payment_status=success&event_id={event.id}",
            cancel_url=f"{frontend_url}?payment_status=cancel",
        )
        
        return jsonify({'requires_stripe': True, 'url': checkout_session.url}), 200

    except Exception as e:
        print(f"❌ Errore creazione sessione Stripe: {str(e)}")
        return jsonify({'message': f'Errore nella creazione del pagamento: {str(e)}'}), 500

# =========================================================================
# 🌟 NUOVO ENDPOINT: CONFERMA REGISTRAZIONE UTENTE DOPO STRIPE
# =========================================================================
@events_bp.route('/api/events/<int:event_id>/confirm-payment', methods=['POST'])
@token_required
def confirm_payment(event_id):
    event = Event.query.get_or_404(event_id)
    user_id = request.user.get('sub') or request.user.get('id')

    # Controlliamo se non sia già stato inserito per evitare duplicati causati da refresh della pagina
    existing_reg = db.session.query(event_registrations).filter_by(event_id=event_id, user_id=str(user_id)).first()
    if existing_reg:
        return jsonify({'message': 'Pagamento già convalidato in precedenza.'}), 200
    ...
    try:
        ...
        event.tickets_sold += 1
        db.session.commit()
        return jsonify({'message': 'Pagamento confermato e biglietto registrato correttamente!'}), 200
    except Exception as e:
        db.session.rollback()
        print(f"❌ Errore registrazione DB post-pagamento: {str(e)}")
        return jsonify({'message': f'Errore durante la memorizzazione del biglietto: {str(e)}'}), 500
